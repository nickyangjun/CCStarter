...
run_smoke_cases() {
  local body headers status token req_body
  ...
  log_api_request "GET" "/api/sample/ping" ""
  ...
  log_api_response "ping" "$status" "$body"
  ...
  status="$(echo "$headers" | head -1 | awk '{print $2}')"
  body="$(cat /tmp/smoke-ping-auto.json)"
  log_api_response "ping (无 Trace 头)" "$status" "$body"
  ...
  log_info "响应头 X-Trace-Id 示例: ${auto_trace} / ${auto_trace_2}"

  log_info "=== 3. 短信验证码登录获取 JWT（test.fixed-code）==="
  req_body='{"mobile":"13800138000","code":"123456"}'
  log_api_request "POST" "/api/auth/sms/login" "$req_body"
  body="$(curl -sS -w '\n%{http_code}' -X POST "${base_url}/api/auth/sms/login" \
  ...
  if [[ "$status" == "401" && "$(json_field "$body" code)" == "UNAUTHORIZED" ]]; then
    log_api_response "sms login" "$status" "$body"
    die "sms login 返回 401：/api/auth/sms/login 未在白名单或 sample 为旧 JAR。请停掉 18080 进程后执行: ./scripts/build.sh && ./scripts/run-sample.sh"
  fi
  log_api_response "sms login" "$status" "$body" 1
  assert_http_status "200" "$status" "sms login"
  token="$(json_field "$body" accessToken)"
  [[ -n "$token" ]] || die "sms login 未返回 accessToken"
  log_info "摘要: userId=$(json_field "$body" userId) username=$(json_field "$body" username) expireMinutes=$(json_field "$body" expireMinutes)"

  log_info "=== 4. 受保护接口（带 Token）==="
  log_api_request "GET" "/api/sample/secure/hello" ""
  body="$(curl -sS -w '\n%{http_code}' \
    -H "X-Trace-Id: ${trace_id}" \
  ...
  status="${body##*$'\n'}"
  body="${body%$'\n'*}"
  log_api_response "secure/hello" "$status" "$body"
  ...
  status="${body##*$'\n'}"
  body="${body%$'\n'*}"
  log_api_response "secure/hello (无 Token)" "$status" "$body"
  ...
  log_api_request "GET" "/api/sample/error/runtime" ""
  body="$(curl -sS -w '\n%{http_code}' \
    -H "X-Trace-Id: ${trace_id}" \
    "${base_url}/api/sample/error/runtime")"
  status="${body##*$'\n'}"
  body="${body%$'\n'*}"
  log_api_response "error/runtime" "$status" "$body"
  ...
  log_api_request "GET" "/api/sample/error/not-exists" ""
  body="$(curl -sS -w '\n%{http_code}' \
    -H "X-Trace-Id: ${trace_id}" \
    "${base_url}/api/sample/error/not-exists")"
  status="${body##*$'\n'}"
  body="${body%$'\n'*}"
  log_api_response "not-exists" "$status" "$body"
  ...
  log_api_request "POST" "/api/sample/orders" ""
  body="$(curl -sS -w '\n%{http_code}' -X POST \
    -H "X-Trace-Id: ${trace_id}" \
    -H "Authorization: Bearer ${token}" \
  ...
  status="${body##*$'\n'}"
  body="${body%$'\n'*}"
  log_api_response "POST orders" "$status" "$body"
  ...
  if [[ "${SMOKE_LOG_CHECK:-0}" == "1" && -n "${smoke_log}" ]]; then
    ...
    if [[ -f "${smoke_log}" && -s "${smoke_log}" ]]; then
      ...
      assert_contains "${log_content}" "userId=1001" "smoke log userId after sms login"
